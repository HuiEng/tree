#ifndef __PARTREE_MULTI_MAIN_CMDLINE_HPP__
#define __PARTREE_MULTI_MAIN_CMDLINE_HPP__
...
class partree_multi_main_cmdline
{
...
public:
  ...
  partree_multi_main_cmdline() : input_arg(""),
                                 output_arg(0), capacity_arg(0), order_arg(0),
                                 input_given(false),
                                 output_given(false), capacity_given(false),
                                 print_tree_arg(false), intersect_arg(false),
                                 query_given(false), query_arg(""),
                                 threshold_given(false), threshold_arg(0)
  {
  }

  partree_multi_main_cmdline(int argc, char *argv[]) : input_arg(""),
                                                       output_arg(0), capacity_arg(0), order_arg(0),
                                                       input_given(false),
                                                       output_given(false), capacity_given(false),
                                                       print_tree_arg(false), intersect_arg(false),
  ...
  void parse(int argc, char *argv[])
  {
    static struct option long_options[] = {
        ...
        {"version", 0, 0, 'V'},
        {0, 0, 0, 0}};
    static const char *short_options = "hVi:o:c:pIq:t:O:";
...
#define CHECK_ERR(type, val, which)                                                      \
  ...
  {                                                                                      \
    ...
    while (true)
    {
      ...
      case 'O':
        ...
        CHECK_ERR(size_t, optarg, "-w, --treeOrder=size_t")
        break;
      case 'p':
        print_tree_arg = true;
        break;
  ...
  static const char *usage() { return "Usage: bftree partreeM [options] "; }
  class error
  {
    int code_;
  ...
  {
    return "Read minimisers BF and get all-against-all Jaccard partree\n\n"
           "Options (default value in (), *required):\n"
           " -i, --input                              input file\n"
           " -o,                                      output cluster file [default=cluster.txt]\n"
           " -c,                                      tree capacity [default=1000000]\n"
           " -O,                                      tree order [default=10]\n"
           " -q, --query                              query file\n"
           " -t, --threshold                          split root if BFI < BF_len * threshold [default=0]\n"
           " -p, --tree                               print tree in Json [default=FALSE]\n"
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void partree_multi(::std::ostream &os = std::cout)
  {
    ...
    os << " order_given:" << order_given << "\t"
       << " order_arg:" << order_arg << "\n";
    os << " print_tree_arg:" << print_tree_arg << "\n";
...
#endif // __PARTREE_MULTI_MAIN_CMDLINE_HPP__"
