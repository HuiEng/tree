...
public:
  ...
  merge_main_cmdline() : bf_input_arg(""), output_arg(""),
                              output_given(false)
  {
  }

  merge_main_cmdline(int argc, char *argv[]) : bf_input_arg(""), output_arg(""),
                                                    output_given(false)
  {
    parse(argc, argv);
  }
  ...
  void parse(int argc, char *argv[])
  {
    static struct option long_options[] = {
        ...
        {"version", 0, 0, 'V'},
        {0, 0, 0, 0}};
    static const char *short_options = "hVo:";
...
#define CHECK_ERR(type, val, which)                                                      \
  ...
  static const char *usage() { return "Usage: tree merge [options] "; }
  class error
  {
    int code_;
  ...
  {
    return "merge result table, assuming the index are the same\n\n"
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void merge(::std::ostream &os = std::cout)
  {
