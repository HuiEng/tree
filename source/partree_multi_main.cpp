#include <random>
#include "partree_multi_main_cmdline.hpp"
// #include "pkm.hpp"
// #include "part_ktree_multi3.hpp"
// typedef part_KTree_multi3 tree_type;
#include "treap2.hpp"
typedef treap tree_type;
using namespace std;

static partree_multi_main_cmdline args; // Command line switches and arguments
...
// tree_type clusterSignatures(const vector<vector<vector<cell_type>>> &seqs, bool printTree, size_t threshold, vector<size_t> &clusters)
// {
//     size_t seqCount = seqs.size();
//     // cout << seqCount << "\n";
//     // vector<size_t> clusters(seqCount);
//     tree_type tree(partree_capacity, threshold);
...
//     //     clusters[i] = tree.first_insert(seqs[i], insertionList, i);
//     // }

//     // for (size_t i = 0; i < seqCount; i++)
//     // {
//     //     clusters[i] = tree.insert(seqs[i], insertionList, i, fase);
//     // }
...
//         // tree.createRoot(seqs[foo[0]]);

//         for (size_t i = 0; i < seqCount; i++)
//         {
//             fprintf(stderr, "inserting %zu at", foo[i]);
//             // clusters[foo[i]] = tree.insert(seqs[foo[i]], insertionList, foo[i]);
//             tree.insert(seqs[foo[i]], insertionList, foo[i]);
//         }
...
//         for (size_t i = 0; i < seqCount; i++)
//         {
//             // fprintf(stderr, "searching %zu", i);
//             clusters[foo[i]] = tree.find(seqs[foo[i]]);
//         }
...
//             tree.clearMatrices(insertionList[0]);
...
//     else
//     {
//         // tree.first_insert(seqs[foo[0]], insertionList, foo[0], false);

//         for (size_t i = 0; i < seqCount; i++)
//         {
//             tree.insert(seqs[foo[i]], insertionList, foo[i], false);
//         }
//         // for (size_t i = 0; i < seqCount; i++)
//         // {
//         //     // fprintf(stderr, "inserting %zu", i);
//         //     tree.insert_split(rng, seqs[foo[i]], insertionList);
//         // }

//         for (size_t i = 0; i < seqCount; i++)
//         {
//             clusters[foo[i]] = tree.insert_again(seqs[foo[i]], foo[i]);
//         }
//     }

//     // tree.Trim(insertionList);

//     // print tree in Json for visualisation
//     if (printTree)
//     {
//         fprintf(stdout, "\nvar treeData=");
...
// void search(PKM tree, const vector<vector<vector<cell_type>>> &queries)
// {
...
int partree_multi_main(int argc, char *argv[])
{
    ...
    {
        HD_threshold = args.threshold_arg;
    }
    ...
    // for (vector<vector<cell_type>> seq : seqs)
    // {
    //     // fprintf(stderr, "seq:\n");
    //     for (vector<cell_type> window : seq)
    //     {
    //         // fprintf(stderr, "%zu>",countSetBits(window));
    //         printBF(stderr, window);
    //         fprintf(stderr, ">");
    //     }
    //     fprintf(stderr, "\n");
    // }
    ...
    // vector<size_t> clusters(seqs.size());
    // tree_type tree = clusterSignatures(seqs, args.print_tree_arg, args.threshold_arg, clusters);

    // treaptest();

    // size_t nums[] = {1, 7, 6, 4, 3, 2, 8, 9, 10};
    tree_type tree(partree_capacity, args.threshold_arg, seqs[0]);
    size_t seqCount = seqs.size();
    vector<size_t> clusters(seqCount);
    ...
    for (size_t n = 1; n < seqCount; n+=26)
    {
        ...
        // size_t best = -1;
        // findNod(tree.root, seqs[n], best, 0, numeric_limits<size_t>::max());
        // fprintf(stderr, "---%zu\n", best);
        // clusters[n] = best;
        clusters[n] = findNod(tree.root, seqs[n]);
        fprintf(stderr, "find %zu at %zu\n", n, clusters[n]);
    }
    compressClusterList(clusters);
    ...
    // tree.outputClusterbyLeaf(pFile, 0);

    // tree.updateMeanSig(1);

    // if (args.query_given)
    // {
    //     fprintf(stderr, "Searching queries...\n");
    //     vector<vector<vector<cell_type>>> queries = readPartitionBF(args.query_arg);
    //     for (vector<vector<cell_type>> q : queries)
    //     {
    //         tree.search(q);
    //     }
    // }
    ...
    //     }
    // }
    // fprintf(stderr, "Size %zu, %zu...\n", signatureSize, maxWindow);
    // kMeans(rng, seqs, args.capacity_arg);

    // auto ans = calcUnion(seqs);
    // for (int w = 0; w < maxWindow; w++)
    // {
    //     for (int i = 0; i < seqs.size(); i++)
    //     {
    //         printBF(stderr, seqs[i][w]);
    //         fprintf(stderr, "\n");
    //     }
    //     fprintf(stderr, ">>>\n");
    //     printBF(stderr, ans[w]);
    //     fprintf(stderr, "---\n");
    // }
