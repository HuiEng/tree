 ...
 * \brief Provides seqan3::views::partition_hash.
 */
...
{
//!\brief seqan3::views::partition_hash's range adaptor object type (non-closure).
//!\ingroup search_views
struct partition_hash_fn
{
    ...
    */
                              ...
                              seed const seed = seqan3::seed{0x8F3F73B5CF1C9ADE}) const
    {
        static_assert(std::ranges::viewable_range<urng_t>,
            "The range parameter to views::partition_hash cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng_t>,
            "The range parameter to views::partition_hash must model std::ranges::forward_range.");
        static_assert(semialphabet<std::ranges::range_reference_t<urng_t>>,
            "The range parameter to views::partition_hash must be over elements of seqan3::semialphabet.");
        ...
        return seqan3::detail::partition_view(forward_strand, reverse_strand, window_size.get() - shape.size() + 1, shape.size());
    }
};
 ...
 * returned for one window. This process is repeated over every possible partition of a sequence. 
 ...
 *
 * \sa seqan3::views::partition_view
 *
 * \attention
 ...
 * ### Example
 *
 * \include test/snippet/search/views/partition_hash.cpp
 *
 * \hideinitializer
 *
 * \experimentalapi{Experimental since version 3.1.}
 */
inline constexpr auto partition_hash = detail::partition_hash_fn{};
