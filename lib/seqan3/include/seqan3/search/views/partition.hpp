...
namespace seqan3::detail
{
    ...
    class partition_view : public std::ranges::view_interface<partition_view<urng1_t, urng2_t>>
    {
    private:
        static_assert(std::ranges::forward_range<urng1_t>, "The partition_view only works on forward_ranges.");
        static_assert(std::ranges::forward_range<urng2_t>, "The partition_view only works on forward_ranges.");
        ...
        //!\brief The sentinel type of the partition_view.
    ...
    public:
         ...
         * \{
         */
        partition_view() = default;                                     //!< Defaulted.
        partition_view(partition_view const &rhs) = default;            //!< Defaulted.
        partition_view(partition_view &&rhs) = default;                 //!< Defaulted.
        partition_view &operator=(partition_view const &rhs) = default; //!< Defaulted.
        partition_view &operator=(partition_view &&rhs) = default;      //!< Defaulted.
        ~partition_view() = default;                                    //!< Defaulted.
         ...
         */
        partition_view(urng1_t urange1, size_t const window_size, size_t const kmerLength) : partition_view{std::move(urange1), default_urng2_t{}, window_size, kmerLength}
        {
        }
        ...
        */
        ...
        requires(std::ranges::viewable_range<other_urng1_t> &&
            ...
            //!\endcond
            partition_view(other_urng1_t &&urange1, size_t const window_size, size_t const kmerLength) : urange1{std::views::all(std::forward<other_urng1_t>(urange1))},
         ...
         * \param[in] kmerLength  The length of kmers.
         */
        partition_view(urng1_t urange1, urng2_t urange2, size_t const window_size, size_t const kmerLength) : urange1{std::move(urange1)},
        ...
        */
        ...
        requires(std::ranges::viewable_range<other_urng1_t> &&
            ...
            //!\endcond
            partition_view(other_urng1_t &&urange1, other_urng2_t &&urange2, size_t const window_size, size_t const kmerLength) : urange1{std::views::all(std::forward<other_urng1_t>(urange1))},
    ...
    template <bool const_range>
    class partition_view<urng1_t, urng2_t>::basic_iterator
    {
    private:
    ...
    public:
        ...
        */
        basic_iterator(urng1_iterator_t urng1_iterator,
                       ...
                       size_t window_size,
                       size_t kmerLength) : urng1_iterator{std::move(urng1_iterator)},
        ...
        //!\brief Compare to the sentinel of the partition_view.
    ...
    partition_view(rng1_t &&, size_t const window_size, size_t const kmerLength) -> partition_view<std::views::all_t<rng1_t>>;

    //!\brief A deduction guide for the view class template.
    template <std::ranges::viewable_range rng1_t, std::ranges::viewable_range rng2_t>
    partition_view(rng1_t &&, rng2_t &&, size_t const window_size, size_t const kmerLength) -> partition_view<std::views::all_t<rng1_t>,
                                                                                                              std::views::all_t<rng2_t>>;

    // ---------------------------------------------------------------------------------------------------------------------
    // partition_fn (adaptor definition)
    // ---------------------------------------------------------------------------------------------------------------------
    ...
    struct partition_fn
    {
        ...
        constexpr auto operator()(size_t const window_size, size_t const kmerLength) const
        {
            return adaptor_from_functor{*this, window_size, kmerLength};
        }
        ...
        constexpr auto operator()(urng1_t &&urange1, size_t const window_size, size_t const kmerLength) const
        {
            ...
            return partition_view{urange1, window_size, kmerLength};
        }
    };
    //![adaptor_def]
