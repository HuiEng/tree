...
size_t BFunion(const cell_type *a, const cell_type *b, size_t signatureSize)
{
   size_t c = 0;
   for (size_t i = 0; i < signatureSize; i++)
   {
...
public:
   ...
   // inline size_t calcHD(const bloom_filter& a, const bloom_filter& b)
   // {
   //    size_t c = 0;
   //    for (size_t i = 0; i < a.bit_table_.size(); i++) {
   //       c += __builtin_popcountll(a.bit_table_[i] ^ b.bit_table_[i]);
   //    }
   //    return c;
   // }
   ...
   inline void insert(const unsigned char *key_begin, const std::size_t &length)
   {
      ...
      // auto parts = table_size_ / salt_.size();
      // //
      // for (size_t i = 0; i < digests.size(); ++i)
      // {
      //    auto temp = i * parts + (digests[i] % parts);
      //    auto bit = temp % bits_per_char;
      //    auto bit_index = (temp - bit) / bits_per_char;
      //    bit_table_[bit_index] |= bit_mask[bit];
      // }
   ...
   inline const unsigned long long int table_size() const
   {
   ...
   inline void print(std::ofstream &wf)
   {
      // wf.write(reinterpret_cast<const char*>(&(bit_table_[i])), sizeof(unsigned char));
      wf.write((char *)&bit_table_[0], bit_table_.size());
   }

   inline void print()
   {
      for (auto t : bit_table_)
      {
         fprintf(stdout, "%d,", t);
      }
      fprintf(stdout, "\n");
