...
#ifndef INCLUDE_part_KTree_MULT3_HPP
#define INCLUDE_part_KTree_MULT3_HPP
...
public:
    ...
    vector<int> isSingleton;                            // n entries, is this a branch node
    vector<int> isLeafNode;                             // n entries, is this a leaf node
    vector<int> hasSiblingNode;                         // n entries, is this a leaf node
    ...
    void reserve(size_t capacity)
    {
        ...
        //#pragma omp parallel
        {
            ...
            //#pragma omp single
            {
                hasSiblingNode.resize(capacity);
            }
            //#pragma omp single
            {
    ...
    part_KTree_multi3(size_t capacity, size_t threshold)
    {
        reserve(capacity);
        childCounts[root] = 0;
        isBranchNode[root] = 0;
    ...
    void spawnSibling(size_t node, vector<vector<cell_type>> signature, vector<size_t> &insertionList)
    {
        size_t sibling = getNewNodeIdx(insertionList);
        fprintf(stderr, "sibling %zu\n", sibling);
        size_t parent = parentLinks[node];

        parentLinks[sibling] = parent;
        meanSigs[sibling] = signature;
        hasSiblingNode[sibling] = 1;

        hasSiblingNode[node] = 1;

        childLinks[parent].push_back(sibling);
        childCounts[parent]++;
    }

    void spawnChild(size_t parent, vector<vector<cell_type>> signature, vector<size_t> &insertionList)
    {
        size_t node = getNewNodeIdx(insertionList);
        fprintf(stderr, "child %zu\n", node);

        parentLinks[node] = parent;
        meanSigs[node] = signature;
    ...
    void printRoot(vector<vector<cell_type>> signature, size_t node, vector<size_t> &HD, vector<size_t> &matching, vector<size_t> &best)
    {

        if (isBranchNode[node])
        {
            size_t hd = compareSeqsHD(meanSigs[node], signature);
            size_t mat = compareSeqsWindows(meanSigs[node], signature);
            HD.push_back(hd);
            matching.push_back(mat);
            best.push_back(node);
            // if (mat >= *matching)
            // {
            //     memcpy(matching, &mat, sizeof(size_t));
            //     memcpy(best, &node, sizeof(size_t));
            //     if (hd < *HD)
            //     {
            //         memcpy(HD, &hd, sizeof(size_t));
            //     }
            // }
            // fprintf(stderr, "%zu,%zu,%zu,%zu\n", node, hd, mat, *best);
            ...
            for (size_t child : childLinks[node])
            {
                ...
                {
                    printRoot(signature, child, HD, matching, best);
                }
                else
                {
                    size_t hd = compareSeqsHD(meanSigs[child], signature);
                    size_t mat = compareSeqsWindows(meanSigs[child], signature);
                    HD.push_back(hd);
                    matching.push_back(mat);
                    ...
                    // {
                    //     memcpy(matching, &mat, sizeof(size_t));
                    //     memcpy(best, &child, sizeof(size_t));
                    //     if (hd < *HD)
                    //     {
                    //         memcpy(HD, &hd, sizeof(size_t));
                    //     }
    ...
    void findBest(vector<vector<cell_type>> signature, size_t node, size_t *HD, size_t *matching, size_t *best)
    {
        ...
        if (isBranchNode[node])
        {
            ...
            for (size_t child : childLinks[node])
            {
                if (isBranchNode[child])
                {
                    findBest(signature, child, HD, matching, best);
                }
                else
                {
                    size_t hd = compareSeqsHD(meanSigs[child], signature);
    ...
    {
        size_t node = root;
        size_t numWindows = signature.size();
        size_t maxWindows = 0;
        size_t minHD = numeric_limits<size_t>::max();
        size_t bestLeafMatch = 0;
        size_t bestLeaf = 0;
        ...
        while (isBranchNode[node])
        {
            ...
            size_t maxWindows = signature.size() / 2;
            minHD = numeric_limits<size_t>::max();
            for (size_t leaf : childLinks[node])
            {
                ...
                // return if all windows matches, ignore HD at this stage
                if (matching == numWindows)
                {
                    return leaf;
                }

                // or else find best matching & lowest HD among child
                if (matching >= maxWindows)
                {
                    maxWindows = matching;
                    bestLeafMatch = leaf;

                    // if multiple leaves with the max matching windows, choose the one with lowest HD
                    if (HD < minHD)
                    {
                        minHD = HD;
                        bestLeaf = leaf;
    ...
    int traverseAny(vector<vector<cell_type>> signature, size_t node)
    {
        // size_t node = root;
        size_t maxWindows = signature.size() / 2;
        size_t bestLeaf = root;
        ...
        if (isBranchNode[node])
        {
            ...
            // all children does not match, find in grandchildren
            for (size_t leaf : childLinks[node])
            {
                size_t ans = traverseAny(signature, leaf);
                // fprintf(stderr, "---%zu,%zu\n", leaf,ans);
                if (ans != root)
                {
                    return ans;
    ...
    int traverseAnyHD(vector<vector<cell_type>> signature, size_t node)
    {
        // size_t node = root;
        size_t minHD = signature.size() * HD_threshold;
        size_t bestLeaf = root;
        ...
        if (isBranchNode[node])
        {
            ...
            for (size_t leaf : childLinks[node])
            {
                size_t ans = traverseAnyHD(signature, leaf);
                // fprintf(stderr, "---%zu,%zu\n", leaf,ans);
                if (ans != root)
                {
                    return ans;
    ...
    size_t traverseHD(vector<vector<cell_type>> signature)
    {
        ...
        while (isBranchNode[node])
        {
            ...
            // fprintf(stderr,"\n---%zu,%zu,%zu\n", node, bestLeaf, minHD);
            // // fill breadth first
            // if (hasSiblingNode[bestLeaf])
            // {
            //     fprintf(stderr,"has sibling\n");
            //     return bestLeaf;
            // }
            node = bestLeaf;
        }
    ...
    void insert(vector<vector<cell_type>> signature, vector<size_t> &insertionList, size_t idx, bool store = true)
    {

        // size_t node = traverse(signature);
        size_t node = traverseAny(signature, root);
        // fprintf(stderr, "at %zu\n", node);

        if (node == root) // no match, grow new node at nearest
        {
            // fprintf(stderr, "here\n");
            node = traverseHD(signature);
            if (hasSiblingNode[node])
            {
                // fprintf(stderr, "child\n");
                spawnChild(node, signature, insertionList);
            }
            else
            {
                // fprintf(stderr, "sibling\n");
                spawnSibling(node, signature, insertionList);
            }
        }
        else
        {
    ...
    // void test(vector<vector<cell_type>> signature)
    // {
    //     vector<size_t> HD;
    //     vector<size_t> matching;
    //     vector<size_t> best;
    //     printRoot(signature, root, HD, matching, best);

    //     fprintf(stderr, "%zu,%zu,%zu\n---\n", HD.size(), matching.size(), best.size());

    //     for (int i = 0; i < HD.size(); i++)
    //     {
    //         fprintf(stderr, "%zu,%zu,%zu\n", HD[i], matching[i], best[i]);
    //     }

    //     fprintf(stderr, "found %zu\n", find(signature));

    //     // auto hd = compareSeqsHD(meanSigs[16], meanSigs[30]);
    //     // auto m = compareSeqsWindows(meanSigs[16], meanSigs[30]);
    //     // fprintf(stderr, "hd %zu, m %zu\n", hd, m);
    // }

    void test(vector<vector<vector<cell_type>>> seqs)
    {
        auto seqCount = seqs.size();
        fprintf(stderr, "i,j,m,hd\n");

        for (int i = 0; i < seqCount - 1; i++)
        {
            for (int j = i + 1; j < seqCount; j++)
            {
                fprintf(stderr, "%zu,%zu,%zu,%zu\n", i, j, compareSeqsWindows(seqs[i],seqs[j]), compareSeqsHD(seqs[i],seqs[j]));
            }
        }
    }

    void clearMatrices(size_t last)
    {
        for (size_t i = 0; i < last; i++)
        {

            if (matrices[i].size() == 1)
            {
                isSingleton[i] = 1;
                meanSigs[i].clear();
                fprintf(stderr, "singleton %zu\n", i);
            }
            else
            {
                isSingleton[i] = 0;
    ...
    size_t find(vector<vector<cell_type>> signature)
    {
        size_t HD = numeric_limits<size_t>::max();
        size_t matching = 0;
        size_t best = 0;
        findBest(signature, root, &HD, &matching, &best);
        matrices[best].push_back(signature);
        updateMeanSig(best);
        return best;
    }
